...
class Forum_comment extends StatefulWidget {
  Forum_comment(this.postUID, this.content, this.username, this.profile_img,
      this.forum_img, this.location, this.post_time, this.mood_icon);
...
class Forum_commentState extends State<Forum_comment> {
  ...
  Future<List<Widget>> fetchComment(String postUID) async {
    ...
    await postInstance.doc(postUID).get().then((value) async {
      ...
      for (var comment in value.data()['comments']) {
        ...
        print("comment: $comment");
        CommentList.add(_commentCard(userName, "assets/profile_pic5.png",
            widget.location, timeFromNow(timestamp), content));
      }
    });
  ...
  Widget _commentCard(String username, String profile_img, String location,
      String post_time, String comment) {
    return Card(
      color: Colors.white,
  ...
  void _postBtn() async {
    ...
    String commentUID = await commentData.createComment(
        userUID, widget.postUID, commentController.text);
